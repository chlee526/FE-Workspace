<template>
    <div>
        <div class="util" :class="!getUseageLNB ? 'is-gnb' : ''">
            <!-- 엑셀 다운로드 목록 버튼 -->
            <div v-if="getExcelList && !getUseageLNB" class="excel-list-btn" :class="getUseageLNB ? 'is-lnb' : ''"><CompExcel /></div>
            <hr v-if="getExcelList && !getUseageLNB" />
            <ul>
                <!-- 사용자 이름/비밀번호 변경 -->
                <li>
                    <!-- 비밀번호 변경 기능 사용 안함 -->
                    <span v-if="!getUsePassSet" class="user">
                        ...
                        <span v-if="userDatas?.id?.trim().length && !getUseageLNB">({{ userDatas.id }})</span>
                    </span>
...
export default {
    ...
    computed: {
        ...
        getUsePassSet() {
            return process.env.VUE_APP_USEAGE_PW_SET == 'true';
        },
        getSystemUrl() {
